...
safeSqrt' x
  | x < 0 = Left "Error: input cannot be negative"
  | otherwise = Right (sqrt x)

-- 5. rewrite the safeIndex function using the Either data type
safeIndex' :: [a] -> Int -> Either String a
safeIndex' xs i
  | i < 0 || i >= length xs = Left $ "Error: Cannot access element at index " ++ show i ++ ". Index is out of bounds."
...
chainSafeIndex xs i = case safeIndex xs i of
  Nothing -> Nothing
  Just i' -> safeIndex xs i'

-- 7. rewrite function chainSafeIndex, with Either data type
chainSafeIndex' :: [Int] -> Int -> Either String Int
chainSafeIndex' xs i = case safeIndex' xs i of
  Left innerError -> Left $ "Error in first call! : " ++ innerError
  Right i' -> safeIndex' xs i'