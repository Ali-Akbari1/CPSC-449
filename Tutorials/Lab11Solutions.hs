...
multiplyLists xs ys = zipWith (\x y -> x * y) xs ys
...
filterShortWords n words = filter (\word -> length word >= n) words
...
multiplyByIndex xs = map (\(x, i) -> x * i) (zip xs [0..])
...
filterNoA strs = filter (\s -> not ('a' `elem` s)) strs
...
isPrime n
  ...
  where
    ...
    hasDivisor n divisor
      | divisor * divisor > n = False
      | n `mod` divisor == 0  = True
      | otherwise             = hasDivisor n (divisor + 1)

filterPrimes :: [Int] -> [Int]
filterPrimes xs = filter (not . isPrime) xs
...
maxWithFunction _ [] = error "empty list"
maxWithFunction _ [x] = x
maxWithFunction cmp (x:xs) = maxHelper cmp x xs

-- Hint: Define a helper function to compare and find the max recursively
maxHelper :: (a -> a -> Ordering) -> a -> [a] -> a
maxHelper _ currentMax [] = currentMax 
maxHelper cmp currentMax (y:ys)
  | cmp y currentMax == GT = maxHelper cmp y ys
  | otherwise              = maxHelper cmp currentMax ys