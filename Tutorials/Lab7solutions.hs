...
append (x:xs) ys = x : append xs ys  -- Prepend the head of the first list to the result of appending the tail to the second list
...
indexAt [] _ = Nothing                        -- Return Nothing for an empty list
indexAt (x:_) 0 = Just x                       -- If the index is 0, return the head of the list wrapped in Just
indexAt (_:xs) n 
